...
@Component({
  selector: 'app-login',
  standalone: true,
  imports: [CommonModule, PasswordModule, ButtonModule, FormsModule, DividerModule, InputTextModule, CardModule, DialogModule, ReactiveFormsModule],
...
export class LoginComponent {
  ...
  protected resetSignInForm(): void {
  ...
  signup(): void {
    this.authService.signup(this.signupForm.controls['email'].value, this.signupForm.controls['username'].value, this.signupForm.controls['password'].value).pipe(
      tap({
        next: (success: boolean): void => {
          if (success) {
            this.router.navigate(['/dashboard']);
